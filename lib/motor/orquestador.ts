...
export function arrancarOrquestador(): void {
  ...
  void (async () => {
    ...
    // La suscripción necesita el registro cargado: si se engancha antes, un
    // evento inmediato se consume con `tareas` vacío y su despertar se pierde.
    if (n.arrancado) engancharEventos(obtenerEstado());
    try {
    ...
    // La rehidratación puede sustituir el Estado completo.
    if (n.arrancado) engancharEventos(obtenerEstado());
  })();
}
...
export async function procesarDecisionPropuesta(decision: Decision): Promise<Decision> {
  ...
  const exigirRevisionHumana = (motivo: string): void => {
    const alerta = `Revisión jurídica pendiente: ${motivo} Requiere validación humana.`;
    estado.actualizar(estado.decisiones, d.id, { alertasLegales: [alerta] });
    d.alertasLegales = [alerta];
  };

  const aplicarRevision = (revision: { conforme: boolean; alertas: string[]; sinRevisar?: string }): void => {
    if (revision.sinRevisar) {
      exigirRevisionHumana(revision.sinRevisar);
  ...
  try {
    ...
    if (soloDespliegueInicial) {
      ...
      const revision = await ejecutarConTraza(estado, "asesor_legal", "revision_en_linea", () => revisarLegalidad(d));
      aplicarRevision(revision);
      if (revision.sinRevisar) estado.marcarServicio("Asesor legal", false, revision.sinRevisar);
      else estado.marcarServicio("Asesor legal", true, `${d.alertasLegales?.length ?? 0} alerta(s)`);
    }
  } catch (e) {
    // Sin revisión legal no se permite una salida autónoma: la política ve la
    // alerta y exige validación humana.
    exigirRevisionHumana(mensajeDe(e));
    if (!esAborto(e)) estado.marcarServicio("Asesor legal", false, mensajeDe(e));
...
export async function aprobarDecision(id: string, quien: string, comentario?: string, opciones: { soloAutonomas?: boolean } = {}): Promise<Decision | undefined> {
  ...
  const todasBien = acciones.length === 0 || acciones.every((a) => a.estado === "ejecutada" && a.resultado?.exito !== false);
  const estadoFinal: Decision["estado"] = todasBien ? "ejecutada" : "fallida";
