...
// no basta: las cámaras de carretera confunden niebla, polvo y contraluz.
//
...
import { completarJson, modeloPara, motivoIndisponible, proveedorDisponible, type ImagenLLM } from "../../ia/llm";
...
type Memoria = { enviadas: number[]; rotacion: number; fotogramasAnalizados: Map<string, number> };
type Global = typeof globalThis & { __atalayaVigia?: Memoria };
const g = globalThis as Global;
const memoria = (): Memoria => (g.__atalayaVigia ??= { enviadas: [], rotacion: 0, fotogramasAnalizados: new Map() });

/** Cuántas imágenes se pueden enviar ahora mismo sin pasarse del límite. */
function cupoDisponible(): number {
  const m = memoria();
...
export const agenteVigiaCamaras: Agente = {
  ...
  async ciclo(ctx: ContextoAgente) {
    ...
    if (!camaras.length) {
      ctx.informarTarea("Ninguna cámara en vigilancia ahora mismo");
      return { resumen: "Sin cámaras que analizar" };
    }
    ...
    for (const camara of camaras) {
      ...
      try {
        ...
        if (positivo && anteriorPositivo) {
          ...
          ctx.registrar(
            "camara_positiva",
            `${camara.nombre}: ${analisis.fuego ? "FUEGO" : "humo"} confirmado en dos análisis seguidos (confianza ${Math.round(analisis.confianza * 100)} %). ${analisis.descripcion}`,
            {
              incendioId: camara.incendioId,
              nivel: "critico",
              datos: { camaraId: camara.id, punto: camara.punto, confianza: analisis.confianza, observacionId: observacion.id },
            },
          );
        } else if (positivo) {
          ctx.registrar("agente", `${camara.nombre}: posible ${analisis.fuego ? "fuego" : "humo"} (confianza ${Math.round(analisis.confianza * 100)} %), espero a confirmarlo en el siguiente ciclo`, {
            incendioId: camara.incendioId,
            nivel: "aviso",
