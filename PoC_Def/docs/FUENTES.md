...
| **Móvil en campo** (`/movil`) | `POST /api/camaras/movil` (propio) | No | fotograma cada 15 s | — | `lib/fuentes/camarasMovil.ts` | `vigia_camaras`, página `/movil` |
| **NASA FIRMS** (focos VIIRS) | `https://firms.modaps.eosdis.nasa.gov/api/area/csv/{KEY}/{FUENTE}/-9.5,35.9,4.4,43.9/1` | **Sí** (`FIRMS_MAP_KEY`) | 15 min | **5.000 transacciones / 10 min**; rango máximo 10 días | `lib/fuentes/firms.ts` | `satelite`, `/api/fuentes/satelite` |
